% Compute the lower part of the Hessian matrix 
...
% output:
%     vector: the lower triangular part of the Hessian matrix 
%            corresponding to the prismatic components.

function vector = hessian_trans_down(T_m, T_n, T_e)
z_m = T_m(1:3,3);
z_n = T_n(1:3,3);
p_e = T_e(1:3,4);
p_m = T_m(1:3,4);
vector = cross(z_n, cross(z_m, (p_e - p_m)));
end